...
function vectorDim (size) {
    ...
    return ((' ').repeat(20) + body + ' ').slice(-10);
}
...
const topModule = g => {
    ...
    const demuxInst = (e, ei, tsuffix) => ['rdp_x_demux' + e.targets.length + ' #(']
        .concat(
...
rdp_x_eb1 #(
    ...
    .i_0_dat(edge${ei}${tsuffix}_dat),
    .i_0_req(edge${ei}${tsuffix}_req),
    .i_0_ack(edge${ei}${tsuffix}_ack),
    .clk(clk),
    .reset_n(reset_n)
);
`;
...
const wireInst = (e, ei) => `
assign edge${ei}_0_dat = edge${ei}_dat;
assign edge${ei}_0_req = edge${ei}_req;
assign edge${ei}_ack = edge${ei}_0_ack;
`;
    ...
    const perInitiatorNode = (n, ni) => {
        ...
        return ['// initiator node: ' + ni]
            .concat(assign('i_' + ni + '_dat', edgePrefix + '_dat'))
            .concat(assign('i_' + ni + '_req', edgePrefix + '_req'))
            .concat(assign(edgePrefix + '_ack', 'i_' + ni + '_ack'));
    };
