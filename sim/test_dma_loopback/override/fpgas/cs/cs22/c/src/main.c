...
VMEM_SECTION unsigned int output_buf[32];

VMEM_SECTION unsigned int consume_buf[32];

int main(void)
{
    // for(int i = 0; i < 16; i++) {
    //     output_buf[i] = 0xd000000 | i;
    // }
    int occupancy;
    dma_in_set(VMEM_DMA_ADDRESS(consume_buf), 32);
    ...
    for(int i = 0; i < 32; i++) {
        output_buf[i] = consume_buf[i] | 0x0f000000;
    }
    ...
    dma_out_set(VMEM_DMA_ADDRESS(output_buf), 32);
    // for(int i = 0; i < 1024; i++) {
    //     *pass_fail_0 = i;
    // }
}
